...
wasm-kv-db is a simple and cache-through key-value database whose query layer and schema is realized by Wasm guests.
Its entire content is stored in a cache, and persistent writes are done eventually in a background thread,
all accesses are performed via Wasm guests.
Because of these two factors - cache first and Wasm guests, it is an extremely fast and simple to use storage tool.
This little KV database hosts Wasm guests and via these guests becomes an app.
There is no middle tier, the KV database **IS** the application.
The store is an oblivious system that will pass binary-encode requests to Wasm guests, realize
their various data and environment needs, pass the results back, without being aware of the types of data or business logic.
In addition, Wasm guests have exclusive access to data guaranteeing their safety and consistency.
App creators (tenants) can be 100% certain that data cannot be manipulated in any other way than by the (controlled by
them) Wasm guests.
...
What follows is a vision behind wasm-kv-db:
- reducers are Wasm functions living inside a Wasm guest
- Wasm guest is a Wasm module submitted by tenant, containing a set of reducers and a standard dispatching method
- at any point in time, there is one Wasm module (guest) per tenant
- guests must contain reducers and expose a special function named `execute` that dispatches calls to reducers
- tenants can see the database in an exclusive way, independent of other tenants
- only reducers can write to the database
- HTTP interface can only read the database or call the reducers
